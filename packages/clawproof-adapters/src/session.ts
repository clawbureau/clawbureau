...
export async function createSession(
  ...
  async function proxyLLMCall(params: ProxyLLMCallParams): Promise<ProxyLLMCallResult> {
    ...
    if (typeof providerAuth === 'string' && providerAuth.trim().length > 0) {
      ...
      const key = bearerMatch ? raw.slice(bearerMatch[0].length).trim() : raw;
      headers['X-Provider-API-Key'] = key;
    }

    const proxyTokenConfigured = typeof config.proxyToken === 'string' && config.proxyToken.trim().length > 0;
    ...
    for (const [k, v] of Object.entries(extra)) {
      ...
      // Prevent callers from overriding PoH binding headers generated by this adapter.
      if (lowerK === 'x-run-id') continue;
      if (lowerK === 'x-event-hash') continue;
      if (lowerK === 'x-idempotency-key') continue;

      // When proxyToken is configured, keep proxy auth source unambiguous.
