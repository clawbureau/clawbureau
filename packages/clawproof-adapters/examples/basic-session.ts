/**
 * Example: basic programmatic session with clawproof adapters.
 *
 * Demonstrates how to create a session, record events, proxy LLM calls,
 * and produce a signed proof bundle.
 *
 * Run with:
 *   CLAWPROOF_PROXY_URL=https://proxy.example.com npx tsx examples/basic-session.ts
 */
...
async function main() {
  ...
  const proxyUrl = process.env.CLAWPROOF_PROXY_URL ?? 'https://proxy.example.com';
