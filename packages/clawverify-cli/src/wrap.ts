...
import { join, dirname, basename, relative, delimiter as pathDelimiter } from 'node:path';
...
function findPythonProofedBypassFlag(args: string[]): string | null {
  for (const arg of args) {
    if (!arg) continue;
    if (arg === '--') break;
    if (arg === '--isolated') return '--isolated';
    if (arg === '-c' || arg === '-m' || arg === '-') break;
    if (!arg.startsWith('-')) break;
    if (arg.startsWith('--')) continue;

    const shortFlags = arg.slice(1);
    if (shortFlags.includes('I')) return '-I';
    if (shortFlags.includes('E')) return '-E';
    if (shortFlags.includes('S')) return '-S';
    if (shortFlags.includes('c') || shortFlags.includes('m')) break;
  }

  return null;
}

const PYTHON_PROOFED_SITECUSTOMIZE = String.raw`# Auto-generated by clawsig wrap (AF2-XRT-001).
# Enforces proofed-mode child egress allowlist for Python runtimes.
import os
import socket
...
def _normalize_host(host):
    ...
    if not value:
        return ""
    if "/" in value or value.startswith("unix:"):
        return ""
...
def _host_allowed(host, allowlist):
    ...
    for entry in allowlist:
        if entry and normalized.endswith("." + entry):
            return True
    return False

def _raise_denied(host, port):
    message = f"PRV_EGRESS_DENIED: direct child egress to {host}:{port} is not allowlisted"
    err = PermissionError(message)
    setattr(err, "code", "PRV_EGRESS_DENIED")
    raise err

if _truthy(os.environ.get("CLAWSIG_ENFORCE_EGRESS_ALLOWLIST")):
    raw_allowlist = os.environ.get("CLAWSIG_EGRESS_ALLOWLIST", "")
    allowlist = set()
    for item in raw_allowlist.split(","):
        normalized = _normalize_host(item)
        if normalized:
            allowlist.add(normalized)
    allowlist.update({"127.0.0.1", "localhost", "::1"})

    _orig_create_connection = socket.create_connection
    _orig_socket_connect = socket.socket.connect

    def _clawsig_create_connection(address, *args, **kwargs):
        if isinstance(address, tuple) and len(address) >= 1:
            host = address[0]
            port = address[1] if len(address) >= 2 else 0
            if isinstance(host, str) and not _host_allowed(host, allowlist):
                _raise_denied(host, port)
        return _orig_create_connection(address, *args, **kwargs)

    def _clawsig_socket_connect(self, address):
        if isinstance(address, tuple) and len(address) >= 1:
            host = address[0]
            port = address[1] if len(address) >= 2 else 0
            if isinstance(host, str) and not _host_allowed(host, allowlist):
    ...
    socket.create_connection = _clawsig_create_connection
    socket.socket.connect = _clawsig_socket_connect
`;
...
export async function wrap(
  ...
  if (proofedMode && isPythonCommand(commandName)) {
    const pythonHooksDir = join(tmpDir, 'python-hooks');
    await mkdir(pythonHooksDir, { recursive: true });
    const sitecustomizePath = join(pythonHooksDir, 'sitecustomize.py');
    await writeFile(sitecustomizePath, PYTHON_PROOFED_SITECUSTOMIZE, 'utf-8');
    childEnv['PYTHONPATH'] = [pythonHooksDir, process.env['PYTHONPATH']]
      .filter((entry): entry is string => typeof entry === 'string' && entry.length > 0)
      .join(pathDelimiter);
    childEnv['CLAWSIG_PYTHON_PROOFED_ADAPTER'] = '1';
    diag(
      `\x1b[36m[clawsig]\x1b[0m Python proofed adapter: ACTIVE (sitecustomize=${sitecustomizePath})\n`,
    );
  } else {
