...
async function writeSignedPolicyBundle(workdir: string, orgId: string): Promise<void> {
  ...
  const payload = {
    ...
    bundle_id: 'bundle_wrap_browser_proofed_1',
...
async function buildGatewayReceiptEnvelope(args: {
  ...
  const payload = {
    receipt_version: '1',
    receipt_id: `rcpt_browser_${crypto.randomUUID()}`,
    gateway_id: 'gw_mock_browser',
    provider: 'openai',
    model: 'gpt-4o-mini',
...
describe('AF2-XRT-002 browser/automation proofed adapter parity', () => {
  it.each([
    '--no-proxy-server',
    '--proxy-server=direct://',
  ])('rejects proofed browser flags that bypass mediated egress (%s)', async (flag) => {
    const workdir = await mkdtemp(join(tmpdir(), 'clawsig-wrap-browser-proofed-'));
    const mock = await startMockClawproxy();

    try {
      await writeSignedPolicyBundle(workdir, 'acme');

      const browserBinaryPath = join(workdir, 'chromium');
      await writeFile(
        browserBinaryPath,
        '#!/usr/bin/env python3\nraise SystemExit(0)\n',
        'utf-8',
      );
      await chmod(browserBinaryPath, 0o755);
      ...
      expect(failure?.stderr).toContain('PRV_BROWSER_ADAPTER_BYPASS_FLAG');
  ...
  it('emits policy-bound proofed evidence and verifier-compatible receipts for browser automation runs', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'clawsig-wrap-browser-proofed-'));
    const mock = await startMockClawproxy();
    ...
    try {
      ...
      await writeFile(
        ...
        [
          ...
          '',
          'args = sys.argv[1:]',
          "proxy_arg = next((arg for arg in args if arg.startswith('--proxy-server=')), None)",
          "resolver_arg = next((arg for arg in args if arg.startswith('--host-resolver-rules=')), None)",
          "expected_resolver_arg = '--host-resolver-rules=MAP * ~NOTFOUND,EXCLUDE localhost,EXCLUDE 127.0.0.1,EXCLUDE ::1'",
          '',
          "if proxy_arg is None or not proxy_arg.startswith('--proxy-server=http://127.0.0.1:'):",
          "    print('missing browser proofed adapter launch arguments', file=sys.stderr)",
          '    raise SystemExit(11)',
          '',
          'if resolver_arg != expected_resolver_arg:',
          "    print('unexpected browser proofed resolver rules', file=sys.stderr)",
          '    raise SystemExit(13)',
          '',
          "if os.environ.get('CLAWSIG_BROWSER_PROOFED_ADAPTER') != '1':",
          "    print('missing CLAWSIG_BROWSER_PROOFED_ADAPTER marker', file=sys.stderr)",
          '    raise SystemExit(12)',
          '',
          "http_proxy = os.environ.get('HTTP_PROXY', '')",
          "https_proxy = os.environ.get('HTTPS_PROXY', '')",
          "all_proxy = os.environ.get('ALL_PROXY', '')",
          "if not http_proxy.startswith('http://127.0.0.1:') or https_proxy != http_proxy or all_proxy != http_proxy:",
          "    print('unexpected browser proofed adapter proxy environment', file=sys.stderr)",
          '    raise SystemExit(14)',
          '',
          ...
          "payload = json.dumps({",
          "    'model': 'gpt-4o-mini',",
          "    'messages': [{'role': 'user', 'content': 'hello from browser automation'}],",
          "}).encode('utf-8')",
          ...
          "    print('unexpected proxy response payload', file=sys.stderr)",
          '    raise SystemExit(16)',
          '',
          'raise SystemExit(0)',
          '',
      ...
      expect(proofBundle.payload.metadata?.runner_attestation_receipt?.payload?.policy?.effective_policy_hash_b64u)
        .toBe(policyHash);
      expect(
        typeof proofBundle.payload.metadata?.sentinels?.egress_policy_receipt?.payload?.blocked_attempt_count,
      ).toBe('number');
      ...
      const allowlistedSigners = [
        mock.signerDid,
        typeof (proofBundle as Record<string, unknown>).signer_did === 'string'
          ? ((proofBundle as Record<string, unknown>).signer_did as string)
          : '',
      ].filter((entry) => entry.length > 0);
      const verification = await verifyProofBundle(proofBundle as any, {
        allowlistedReceiptSignerDids: allowlistedSigners,
      });
