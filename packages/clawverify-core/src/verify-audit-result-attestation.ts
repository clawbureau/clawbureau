/**
 * Audit Result Attestation Verification
 * CVF-US-018: Verify audit result attestations (Audit analogue)
 */

import type {
  SignedEnvelope,
  AuditResultAttestationPayload,
  ModelIdentityTier,
  VerificationResult,
  VerificationError,
} from './types.js';
...
import { validateAuditResultAttestationEnvelopeV1 } from './schema-validation.js';
...
function validateEnvelopeStructure(
  envelope: unknown
): envelope is SignedEnvelope<AuditResultAttestationPayload> {
...
export async function verifyAuditResultAttestation(
  envelope: unknown,
  options: AuditResultAttestationVerifierOptions = {}
): Promise<{
  result: VerificationResult;
  audit_id?: string;
  audit_pack_hash_b64u?: string;
  model?: { provider?: string; name?: string; tier?: ModelIdentityTier };
  ...
  if (envelope.envelope_type !== 'audit_result_attestation') {
    return {
      result: {
        status: 'INVALID',
        reason: `Expected audit_result_attestation envelope, got: ${envelope.envelope_type}`,
        verified_at: now,
      },
      error: {
        code: 'UNKNOWN_ENVELOPE_TYPE',
        message: 'This endpoint only accepts audit_result_attestation envelopes',
        field: 'envelope_type',
      },
    };
  }
  ...
  const schemaResult = validateAuditResultAttestationEnvelopeV1(envelope);
  ...
  if (!options.allowlistedSignerDids || options.allowlistedSignerDids.length === 0) {
    return {
      result: {
        status: 'INVALID',
        reason: 'Audit result attestation signer allowlist not configured',
        envelope_type: envelope.envelope_type,
        signer_did: envelope.signer_did,
        verified_at: now,
      },
      error: {
        ...
        message:
          'Audit result attestation signer allowlist is not configured. Set AUDIT_RESULT_ATTESTATION_SIGNER_DIDS to enable verification.',
        field: 'env.AUDIT_RESULT_ATTESTATION_SIGNER_DIDS',
      },
    };
  }
  ...
  if (!options.allowlistedSignerDids.includes(envelope.signer_did)) {
    return {
      ...
      error: {
        ...
        message: `Signer DID '${envelope.signer_did}' is not in the allowlisted audit-result attester list`,
        field: 'signer_did',
      },
    };
  }
  ...
  const auditPackHash =
    isRecord(payloadRec.audit_pack) && typeof payloadRec.audit_pack.pack_hash_b64u === 'string'
      ? payloadRec.audit_pack.pack_hash_b64u
      : undefined;

  const modelSummary = extractModelSummary(payloadRec.model);

  const auditCodeHash =
    isRecord(payloadRec.audit_code) && typeof payloadRec.audit_code.code_hash_b64u === 'string'
      ? payloadRec.audit_code.code_hash_b64u
      : undefined;

  const datasetId =
    isRecord(payloadRec.dataset) && typeof payloadRec.dataset.dataset_id === 'string'
      ? payloadRec.dataset.dataset_id
      : undefined;

  const datasetHash =
    isRecord(payloadRec.dataset) && typeof payloadRec.dataset.dataset_hash_b64u === 'string'
      ? payloadRec.dataset.dataset_hash_b64u
      : undefined;

  const protocolName =
    isRecord(payloadRec.protocol) && typeof payloadRec.protocol.name === 'string'
      ? payloadRec.protocol.name
      : undefined;

  const protocolConfigHash =
    isRecord(payloadRec.protocol) && typeof payloadRec.protocol.config_hash_b64u === 'string'
      ? payloadRec.protocol.config_hash_b64u
      : undefined;

  const resultStatus =
    isRecord(payloadRec.result) && typeof payloadRec.result.status === 'string'
      ? payloadRec.result.status
      : undefined;

  const resultsHash =
    isRecord(payloadRec.result) && typeof payloadRec.result.results_hash_b64u === 'string'
      ? payloadRec.result.results_hash_b64u
      : undefined;

  return {
    result: {
      status: 'VALID',
      reason: 'Audit result attestation verified successfully',
      envelope_type: envelope.envelope_type,
      signer_did: envelope.signer_did,
      verified_at: now,
    },
    audit_id: auditId,
    audit_pack_hash_b64u: auditPackHash,
    model: modelSummary,
    audit_code_hash_b64u: auditCodeHash,
    dataset_id: datasetId,
    dataset_hash_b64u: datasetHash,
