 ...
 *   CLAWSIG_OUTPUT_DIR  — output directory for proof artifacts (default: <repo>/artifacts/poh/<branch>/)
 *
 *   # Optional: offline verification post-run (CPL-US-010)
 *   CLAWSIG_VERIFY        — set to 1 to verify the generated proof bundle offline
 *   CLAWSIG_VERIFY_CONFIG — path to clawverify config (default: packages/schema/fixtures/clawverify.config.clawbureau.v1.json if present)
...
async function loadOrGenerateKeyPair(keyFile: string): Promise<Ed25519KeyPair> {
  try {
    ...
    process.stderr.write(`clawsig: generated new key pair → ${did}\n`);
    process.stderr.write(`clawsig: saved to ${keyFile}\n`);
    return kp;
  }
}
...
export async function main(argv: string[]): Promise<void> {
  ...
  if (dashDash === -1 || dashDash === 0) {
    process.stderr.write(
      `Usage: clawsig-wrap <harness-id> -- <command> [args...]\n\n` +
      ...
      `\n` +
      `  ${ENV.AGENT_KEY_FILE}    — JWK key file (default: <repo>/.clawsig-key.json)\n` +
  ...
  if (command.length === 0) {
    process.stderr.write('clawsig: no command specified after --\n');
    process.exit(1);
    return;
  }
  ...
  if (!adapter) {
    process.stderr.write(
      `clawsig: unknown harness "${harnessId}"\n` +
  ...
  if (!proxyUrl) {
    process.stderr.write(`clawsig: ${ENV.CLAWPROXY_BASE_URL} is required\n`);
    process.exit(1);
    return;
  }
  ...
  if (!proxyToken && (bountiesBaseUrl || bountiesBountyId || bountiesWorkerToken)) {
    if (!bountiesBaseUrl || !bountiesBountyId || !bountiesWorkerToken) {
      process.stderr.write(
        `clawsig: marketplace CST auto-fetch requested but missing env vars. Need ${ENV.CLAWBOUNTIES_BASE_URL}, ${ENV.CLAWBOUNTIES_BOUNTY_ID}, ${ENV.CLAWBOUNTIES_WORKER_TOKEN}.\n`,
      );
      process.exit(1);
      return;
    }
    ...
    process.stderr.write(
      `clawsig: fetching job CST from clawbounties (${bountiesBaseUrl}, bounty=${bountiesBountyId})\n`,
    );
    ...
    try {
      ...
      process.stderr.write(`clawsig: failed to fetch job CST: ${message}\n`);
      process.exit(1);
      return;
    }
  }
  ...
  const keyFileDefault = repoRoot
    ? join(repoRoot, '.clawsig-key.json')
    : '.clawsig-key.json';
  ...
  process.stderr.write(`clawsig: session started (run=${session.runId}, did=${agentDid})\n`);
  process.stderr.write(`clawsig: harness=${adapter.HARNESS.id} v${adapter.HARNESS.version}\n`);
  ...
  const shim = await startShim({
    session,
    log: (msg) => process.stderr.write(`clawsig: ${msg}\n`),
  });
  ...
  try {
    ...
    exitCode = await new Promise<number>((resolve) => {
      ...
      child.on('error', (err: Error) => {
        process.stderr.write(`clawsig: failed to spawn "${command[0]}": ${err.message}\n`);
        resolve(127);
      });
    });
  } finally {
  ...
  process.stderr.write(`\nclawsig: proof bundle → ${bundlePath}\n`);
  process.stderr.write(`clawsig: URM          → ${urmPath}\n`);
  process.stderr.write(`clawsig: trust pulse  → ${trustPulsePath}\n`);
  ...
  if (verifyEnabled) {
    ...
    process.stderr.write(`clawsig: verify       → ${verifyPath}\n`);
    process.stderr.write(
      `clawsig: verify status=${status} reason_code=${reasonCode}\n`,
    );
  ...
  process.stderr.write(
    `clawsig: run=${session.runId} events=${result.envelope.payload.event_chain?.length ?? 0} receipts=${result.envelope.payload.receipts?.length ?? 0}\n`,
  );
...
if (args.length > 0) {
  main(args).catch((err) => {
    process.stderr.write(`clawsig: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
    return;
  });
}
