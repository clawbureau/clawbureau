 ...
 *
 * Used by the OpenClaw clawproxy provider plugin to auto-fetch a job CST
 * (CWC or non-CWC) from clawbounties.
 */
...
export function parseMarketplaceCstResponse(json: unknown): ParsedMarketplaceCst {
  if (!isRecord(json)) {
    throw new Error('clawproxy provider: clawbounties /cst returned an invalid response (expected a JSON object)');
  }
