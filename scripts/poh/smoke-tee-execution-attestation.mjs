...
/**
 * Smoke: POHVN-US-008 tee_execution attestation verification
 *
 * - Valid path: allowlisted TEE root + TCB => VALID
 * - Tampered policy path: unallowlisted TCB => INVALID
 */
...
async function main() {
  ...
  const teeRootId = String(args.get('tee-root-id') || process.env.TEE_ROOT_ID || 'tee_root_sim_v1');
  const teeTcbVersion = String(args.get('tee-tcb-version') || process.env.TEE_TCB_VERSION || 'tee_tcb_sim_v1');
  ...
  const bundleHash = await sha256B64u(`bundle:${envName}:${Date.now()}`);

  const validPayload = {
    attestation_version: '1',
    attestation_id: `execatt_tee_${envName}_${Date.now()}`,
    execution_type: 'tee_execution',
    agent_did: signer.did,
    attester_did: signer.did,
    run_id: `run_tee_${envName}`,
    ...
    runtime_metadata: {
      tee: {
        ...
        root_id: teeRootId,
        tcb_version: teeTcbVersion,
        evidence_ref: {
          resource_type: 'tee_quote',
          resource_hash_b64u: await sha256B64u(`tee-quote:${envName}:${Date.now()}`),
        },
        measurements: {
          measurement_hash_b64u: await sha256B64u(`tee-measure:${envName}:${Date.now()}`),
          runtime_digest_b64u: await sha256B64u(`tee-runtime:${envName}`),
        },
      },
    },
  };

  const validEnvelope = await signEnvelope(validPayload, signer);
  const valid = await httpJson(`${verifyBaseUrl}/v1/verify/execution-attestation`, {
  ...
  assert(valid.json?.tee_root_id === teeRootId, 'valid response missing tee_root_id');
  assert(valid.json?.tee_tcb_version === teeTcbVersion, 'valid response missing tee_tcb_version');

  const invalidPayload = {
    ...validPayload,
    attestation_id: `execatt_tee_${envName}_${Date.now()}_bad`,
    runtime_metadata: {
      ...validPayload.runtime_metadata,
      tee: {
        ...validPayload.runtime_metadata.tee,
        tcb_version: `${teeTcbVersion}_not_allowlisted`,
      },
    },
  };

  const invalidEnvelope = await signEnvelope(invalidPayload, signer);
  const invalid = await httpJson(`${verifyBaseUrl}/v1/verify/execution-attestation`, {
  ...
  assert(invalid.json?.error?.code === 'CLAIM_NOT_FOUND', `tampered expected CLAIM_NOT_FOUND, got ${invalid.text}`);
    ...
    JSON.stringify(
      {
        ...
        signer_did: signer.did,
        tee_root_id: teeRootId,
        tee_tcb_version: teeTcbVersion,
        valid: {
          status: valid.status,
          result_status: valid.json?.result?.status,
          tee_root_id: valid.json?.tee_root_id,
          tee_tcb_version: valid.json?.tee_tcb_version,
        },
        tampered: {
          status: invalid.status,
          result_status: invalid.json?.result?.status,
          error_code: invalid.json?.error?.code,
          field: invalid.json?.error?.field,
        },
      },
      null,
      2,
