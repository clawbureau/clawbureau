...
function validateEnvelopeStructure(
  envelope: unknown
): envelope is SignedEnvelope<OwnerAttestationPayload> {
...
/**
 * Validate owner attestation payload structure
 */
function validateOwnerAttestationPayload(
  payload: unknown
): payload is OwnerAttestationPayload {
  ...
  if (typeof p.attestation_id !== 'string') return false;
  if (typeof p.subject_did !== 'string') return false;

  // Optional fields - validate if present
  if ('provider_ref' in p && typeof p.provider_ref !== 'string') return false;
...
/**
 * Verify an owner attestation envelope
 *
 * Acceptance Criteria:
...
export async function verifyOwnerAttestation(
  ...
  // 4. Verify this is an owner_attestation envelope
  if (envelope.envelope_type !== 'owner_attestation') {
    return {
      result: {
        status: 'INVALID',
        reason: `Expected owner_attestation envelope, got: ${envelope.envelope_type}`,
        verified_at: now,
      },
      owner_status: ownerStatus,
      error: {
        code: 'UNKNOWN_ENVELOPE_TYPE',
        message: 'This endpoint only accepts owner_attestation envelopes',
        field: 'envelope_type',
      },
    };
  }
  ...
  // 7. Validate DID formats
  if (!isValidDidFormat(envelope.signer_did)) {
    return {
      result: {
        status: 'INVALID',
        reason: `Invalid signer DID format: ${envelope.signer_did}`,
        verified_at: now,
      },
      owner_status: ownerStatus,
      error: {
  ...
  // 10. Validate owner attestation payload structure
  if (!validateOwnerAttestationPayload(envelope.payload)) {
    return {
      result: {
        status: 'INVALID',
        reason: 'Invalid owner attestation payload structure',
        verified_at: now,
      },
      owner_status: ownerStatus,
      error: {
        code: 'MALFORMED_ENVELOPE',
        message:
          'Owner attestation payload is missing required fields or has invalid types',
        field: 'payload',
      },
    };
  }
  ...
  if (!isValidDidFormat(envelope.payload.subject_did)) {
    return {
      result: {
        status: 'INVALID',
        reason: `Invalid subject DID format: ${envelope.payload.subject_did}`,
        verified_at: now,
      },
      owner_status: ownerStatus,
      attestation_id: envelope.payload.attestation_id,
      subject_did: envelope.payload.subject_did,
      provider_ref: envelope.payload.provider_ref,
      expires_at: envelope.payload.expires_at,
      error: {
        code: 'INVALID_DID_FORMAT',
        message:
          'Subject DID does not match expected format (did:key:... or did:web:...)',
        field: 'payload.subject_did',
      },
    };
  }

  // Provider reference semantics: missing => unknown, present-but-empty => invalid
  if (envelope.payload.provider_ref !== undefined) {
    if (!isNonEmptyString(envelope.payload.provider_ref)) {
      return {
        result: {
          status: 'INVALID',
          reason: 'Invalid provider_ref (must be non-empty)',
          verified_at: now,
        },
        owner_status: ownerStatus,
        attestation_id: envelope.payload.attestation_id,
        ...
        error: {
          code: 'MALFORMED_ENVELOPE',
          message: 'provider_ref must be a non-empty string when provided',
          field: 'payload.provider_ref',
        },
      };
    }
  ...
  // 11. Check attestation expiry if expires_at is set
  if (envelope.payload.expires_at) {
    ...
    if (expiresAt < currentTime) {
      ...
      return {
        result: {
          status: 'INVALID',
          reason: 'Owner attestation has expired',
          verified_at: now,
        },
        owner_status: ownerStatus,
        attestation_id: envelope.payload.attestation_id,
        ...
        error: {
          code: 'EXPIRED',
          message: `Attestation expired at ${envelope.payload.expires_at}`,
  ...
  return {
    result: {
      ...
      reason:
        ownerStatus === 'verified'
          ? 'Owner attestation verified successfully'
          : 'Owner attestation signature verified (no provider_ref; owner status unknown)',
