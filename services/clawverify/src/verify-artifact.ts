...
function validateEnvelopeStructure(
  envelope: unknown
): envelope is SignedEnvelope<ArtifactPayload> {
...
function validateArtifactPayload(
  ...
  return (
    ...
    typeof p.artifact_type === 'string' &&
    typeof p.content_hash_b64u === 'string' &&
    typeof p.content_type === 'string' &&
    typeof p.content_size_bytes === 'number'
  );
}
...
export async function verifyArtifact(
  envelope: unknown
): Promise<{ result: VerificationResult; error?: VerificationError }> {
  ...
  // 4. Verify this is an artifact_signature envelope
  if (envelope.envelope_type !== 'artifact_signature') {
    return {
      result: {
        status: 'INVALID',
        reason: `Expected artifact_signature envelope, got: ${envelope.envelope_type}`,
        verified_at: now,
      },
      error: {
        code: 'UNKNOWN_ENVELOPE_TYPE',
        message: 'This endpoint only accepts artifact_signature envelopes',
        field: 'envelope_type',
      },
    };
  }
  ...
  // 10. Validate artifact payload structure
  if (!validateArtifactPayload(envelope.payload)) {
    return {
      result: {
        status: 'INVALID',
        reason: 'Invalid artifact payload structure',
        verified_at: now,
      },
      error: {
        code: 'MALFORMED_ENVELOPE',
        message: 'Artifact payload is missing required fields or has invalid types',
        field: 'payload',
      },
    };
  }
  ...
  // 14. All checks passed - signature is valid
  return {
    result: {
      status: 'VALID',
      reason: 'Artifact signature verified successfully',
