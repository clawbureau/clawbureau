...
async function makeVirCase(args?: {
  modelClaimed?: string;
  ...
  const runId = 'run_vir_consistency_001';

  const eventPayloadHash = await computeHash({ type: 'llm_call' }, 'SHA-256');
  const eventHeader = {
    event_id: 'evt_vir_consistency_001',
    run_id: runId,
    event_type: 'llm_call',
  ...
  const disclosedLeaves: Record<string, { type: 'string' | 'number' | 'boolean' | 'null'; value: unknown; salt_b64u: string }> = {
    model_claimed: { type: 'string', value: modelClaimed, salt_b64u: 'salt_model_claimed_consistency_001' },
    model_observed: { type: 'string', value: modelObserved, salt_b64u: 'salt_model_observed_consistency_001' },
    tokens_input: { type: 'number', value: 321, salt_b64u: 'salt_tokens_input_consistency_001' },
    tokens_output: { type: 'number', value: 654, salt_b64u: 'salt_tokens_output_consistency_001' },
  };
  ...
  for (const key of Object.keys(disclosedLeaves).sort((a, b) => a.localeCompare(b))) {
    ...
    const value = args?.tamperLeafValue && key === 'tokens_input' ? 322 : leaf.value;
  ...
  const virPayload: Record<string, unknown> = {
    receipt_version: '2',
    receipt_id: 'vir_consistency_001',
    source: 'gateway',
    provider: 'openai',
    model: modelObserved,
    model_claimed: modelClaimed,
    model_observed: modelObserved,
    request_hash_b64u: 'req_hash_consistency_001',
    response_hash_b64u: 'res_hash_consistency_001',
    tokens_input: 321,
    tokens_output: 654,
    latency_ms: 111,
    agent_did: agent.did,
    ...
    binding: {
      ...
      nonce: 'nonce_consistency_001',
      subject_did: 'did:key:zSubjectConsistency001',
      scope_hash_b64u: 'scope_hash_consistency_001',
    },
    legal_binding: {
      nonce: 'nonce_consistency_001',
      subject_did: 'did:key:zSubjectConsistency001',
  ...
  const bundlePayload: Record<string, unknown> = {
    bundle_version: '1',
    bundle_id: 'bundle_vir_consistency_001',
    agent_did: agent.did,
    event_chain: [
      {
        ...eventHeader,
  ...
  return { virEnvelope, bundleEnvelope };
}

describe('VIR core consistency across receipt and bundle paths', () => {
  it.each([
    ['ERR_MERKLE_ROOT_MISMATCH', { tamperLeafValue: true }],
    [
  ...
  ])('produces INVALID + EVIDENCE_MISMATCH for %s in both callers', async (_code, args) => {
    const { virEnvelope, bundleEnvelope } = await makeVirCase(args as any);

    const receiptResult = await verifyReceipt(virEnvelope);
    const bundleResult = await verifyProofBundle(bundleEnvelope);

    expect(receiptResult.result.status).toBe('INVALID');
    expect(receiptResult.error?.code).toBe('EVIDENCE_MISMATCH');

    expect(bundleResult.result.status).toBe('INVALID');
    expect(bundleResult.error?.code).toBe('EVIDENCE_MISMATCH');
  });
});
